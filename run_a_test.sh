...
echo "[INFO] Golden-miner использует все CPU, потоков на карту: $GM_THREADS_PER_GPU"
echo "[INFO] Worker.jar будет ограничен через taskset на оставшихся ядрах"

# === Запуск golden-miner (БЕЗ taskset, лог в консоль) ===
bash -c "while true; do
    \"$WORKER_BIN\" \
        ...
        --pubkey=\"$PUBKEY\"
    EXIT_CODE=\$?
    echo \"❌ golden-miner остановился (\$EXIT_CODE). Перезапуск через 5 сек...\"
    sleep 5
done" &

# === Функция запуска worker.jar с CPU affinity (в фоне) ===
start_worker_jar() {
    local threads=$1
    local cpus=$2
...
# === Распределение CPU для worker.jar ===
